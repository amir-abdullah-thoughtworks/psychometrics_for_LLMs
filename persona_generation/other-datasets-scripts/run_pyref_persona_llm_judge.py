...
if __name__ == "__main__":
    ...
    reft_config = pyreft.ReftConfig(representations={"layer": 15, "component": "block_output", "low_rank_dimension": 8092, "intervention": pyreft.LoreftIntervention(embed_dim=model.config.hidden_size, low_rank_dimension=8092)})
    reft_model = pyreft.get_reft_model(model, reft_config)
    reft_model.set_device("cuda:0")
    ...
    #print(dataset[0])
    data_module = pyreft.make_last_position_supervised_data_module(tokenizer, model, [prompt_no_input_template % e[0] for e in dataset], [e[1] for e in dataset])
    #print(data_module)

    training_args = transformers.TrainingArguments(num_train_epochs=1, output_dir="/tmp", report_to = "wandb", per_device_train_batch_size=10, 
    ...
    cv_temperature = 1.0
    filename = "pyreft_personachat_unsupervisedPCA_mistral7b_mistralchat_newtemplate_10epochs"
    ...
    if prompt_template in ["mistral-perso-pairwise", "mistral_perso_strict_pairwise"]:
        for input_context in tqdm(dataset):
            ...
            base_unit_location = input_ids["input_ids"].shape[-1] - 1
            _, reft_response = reft_model.generate(
            input_ids, unit_locations={"sources->base": (None, [[[base_unit_location]]])},
            intervene_on_prompt=True, max_new_tokens=128, do_sample=True, 
            eos_token_id=tokenizer.eos_token_id, early_stopping=True
)
            controlled_response = tokenizer.decode(reft_response[0], skip_special_tokens=True)
