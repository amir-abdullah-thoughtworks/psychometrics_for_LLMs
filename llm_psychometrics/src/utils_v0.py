...
def list_to_str(question_list: List):
    return " \n".join([f"{i + 1}. {value}" for i, value in enumerate(question_list)])
...
def generate_combinations(dict1, dict2):
    ...
    return [
        ({k: v for k, v in zip(keys1, combo1)}, {k: v for k, v in zip(keys2, combo2)})
...
def generate_mcq_per_statement(
    statement,
    trait,
    persona_prompt="You are a helpful assistant. Always respond with valid JSON. No explanations.",
    n=10,
    ...
    statement_to_question_api_response = openai_api_call(
        persona_prompt, statement_to_question_prompt(statement, n=n)
    )
    ...
    for question in generated_questions:
        mcq_question_openai_response = openai_api_call(
            persona_prompt, question_to_mcq_prompt(question, trait)
        )
        try:
